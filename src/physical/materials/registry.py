...
Materials are defined once (as the Pydantic ``MaterialProperties`` subclasses in
this package) and referenced by both pipeline stages: the CAD/mesh stage matches
STEP region names against ``tag``; the sif writer reads the same objects' Elmer
keywords. There is deliberately no second ``known_materials = ["N52", ...]`` list
(that lived in the now-deleted ``geometry/mesh.py``) — this registry is the one
source of truth, keyed by each material's ``tag``.
"""
...
# Each value is a zero-arg factory so callers get a fresh, independent instance
# (materials are mutable Pydantic models; sharing one instance across configs
# would let an edit in one place leak into another).
...
def register_material(tag: str, factory: type[MaterialProperties]) -> None:
    """Add (or replace) a material in the registry. Lets downstream code extend
    the vocabulary without editing this module."""
